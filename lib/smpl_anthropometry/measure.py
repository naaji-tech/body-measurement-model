...
class Measurer:
    ...
    def measure(self, measurement_names: List[str]):
        ...
        for m_name in measurement_names:
            ...
            if MEASUREMENT_TYPES.get(m_name) == MeasurementType().LENGTH:
                value = self.measure_length(m_name)
                self.measurements[m_name] = value

            elif MEASUREMENT_TYPES.get(m_name) == MeasurementType().CIRCUMFERENCE:
    ...
    def measure_length(self, measurement_name: str):
        ...
        for i in range(2):
            if isinstance(measurement_landmarks_inds[i], tuple):
                ...
                lm = (
                    self.verts[measurement_landmarks_inds[i][0]]
                    + self.verts[measurement_landmarks_inds[i][1]]
                ) / 2
            else:
...
class MeasureSMPL(Measurer):
    ...
    def __init__(self):
        ...
        # self.body_model_root = "lib\smpl_anthropometry\data"
        ...
        face_segmentation_path = os.path.join(
            self.body_model_path, f"{self.model_type}_body_parts_2_faces.json"
        )
        ...
        self.circumf_2_bodypart = (
            SMPLMeasurementDefinitions().CIRCUMFERENCE_TO_BODYPARTS
        )
        self.all_possible_measurements = (
            SMPLMeasurementDefinitions().possible_measurements
        )
...
class MeasureSMPLX(Measurer):
    ...
    def __init__(self):
        ...
        self.circumf_2_bodypart = (
            SMPLXMeasurementDefinitions().CIRCUMFERENCE_TO_BODYPARTS
        )
        self.all_possible_measurements = (
            SMPLXMeasurementDefinitions().possible_measurements
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--measure_neutral_smpl_with_mean_shape",
        action="store_true",
