...
class Visualizer:
    ...
    def create_landmarks_plot(
        ...
        for lm_name in landmark_names:
            ...
            plot = go.Scatter3d(
                ...
                marker=dict(
                    size=8,
                    color=landmark_colors.get(lm_name),
                    opacity=1,
                ),
                name=lm_name,
            )
    ...
    def create_measurement_length_plot(
        ...
        if measurement_name in self.measurements:
            m_viz_name = (
                f"{measurement_name}: {self.measurements[measurement_name]:.2f}cm"
            )
        else:
    ...
    def create_measurement_circumference_plot(
        ...
        circumf_landmark_indices = [
            self.landmarks[l_name] for l_name in circumf_landmarks
        ]
    ...
    def visualize(
        ...
        measurement_colors = dict(
            zip(self.measurement_types.keys(), px.colors.qualitative.Alphabet)
        )
        ...
        if self.visualize_measurements:
            for m_name in measurement_names:
                ...
                if self.measurement_types.get(m_name) == MeasurementType().LENGTH:
                    ...
                    self.measurement_types.get(m_name)
...
def viz_face_segmentation(
    verts, faces, face_colors, title="Segmented body", name="mesh", show=True
):
    """
    ...
    fig.add_trace(mesh_plot)
    fig.update_layout(scene_aspectmode="data", width=1000, height=700, title=title)
...
def viz_smpl_face_segmentation(fig=None, show=True, title="SMPL face segmentation"):
    ...
    mapping_bp2ind = dict(
        zip(face_segmentation.keys(), range(len(face_segmentation.keys())))
    )
    ...
    fig = viz_face_segmentation(
        verts, faces, face_colors, title=title, name="smpl", show=False
    )
...
def viz_point_segmentation(
    verts, point_segm, title="Segmented body", fig=None, show=True
):
    """
...
def viz_smplx_point_segmentation(fig=None, show=True, title="SMPLX point segmentation"):
    ...
    fig = viz_point_segmentation(
        smpl_verts, point_segm, title=title, fig=fig, show=show
    )
...
def viz_landmarks(
    ...
    for i, (lm_name, lm_ind) in enumerate(landmark_dict.items()):
        plot = go.Scatter3d(
            ...
            z=[verts[lm_ind, 2]],
            mode="markers",
            marker=dict(size=10, color=colors[i], opacity=1, symbol="cross"),
            name=name + "-" + lm_name,
        )
        fig.add_trace(plot)
